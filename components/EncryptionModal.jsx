...
export function EncryptionSetupModal({ open, onClose, onSetup }) {
  ...
  const handleSetup = async () => {
    ...
    // Complexity requirement: at least 4 unique characters
    const uniqueChars = new Set(password).size;
    if (uniqueChars < 4) {
      setError('Password must be more complex (at least 4 unique characters)');
      return;
    }
...
export function ChangePasswordModal({ open, onClose, onChange }) {
  ...
  const handleChange = async () => {
    ...
    if (uniqueChars < 4) {
      setError('New password must be more complex (at least 4 unique characters)');
      return;
    }
