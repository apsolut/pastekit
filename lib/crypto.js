...
const PBKDF2_ITERATIONS = 600000; // OWASP recommended for HMAC-SHA256
...
export async function decrypt(encryptedData, password) {
  ...
  // Try current iterations (600,000)
  let key = await deriveKey(password, salt, PBKDF2_ITERATIONS);
  let decrypted = nacl.secretbox.open(ciphertext, nonce, key);

  // Fallback to legacy iterations (100,000) if decryption failed
  if (!decrypted) {
